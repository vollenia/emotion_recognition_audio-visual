...
#Multimodal Model
class MM(nn.Module): #50e/0.8d=53/48/53% (4), 25e/0.7d=55/48/54% (5) 
	def __init__(self):
		super().__init__()
		...
		self.drop_a = nn.Dropout(p=0.7) #0.7
		self.drop_v = nn.Dropout(p=0.8)
		self.drop_x = nn.Dropout(p=0.7) #0.7
	...
	def convs_a(self, a):
		...
		a = torch.squeeze(a, dim=3) #gets rid of the last dimension (torch.Size([1, 32, 142, 1]) -> torch.Size([1, 32, 142]))
		a = F.max_pool1d(a, 5)
		a = self.drop_a(a)
...
def training(X_train_a, X_train_v, y_train):
	...
	train_correct = 0
	train_total = 0
	for i in range(0, len(X_train_a), BATCH_SIZE): #(start, stop, step) #for i in tqdm(range(0, len(X_train), BATCH_SIZE)): #(start, stop, step)
		#print(i, i+BATCH_SIZE)
		X_train_batch_a = X_train_a[i:i+BATCH_SIZE].view(-1, 1, max_len_features, 23) #.view(-1, 1, max_len_features, 23) for CNN
...
def testing(X_a, X_v, y, final_test=False):
	...
	with torch.no_grad():
		for i in range(0, len(X_a), BATCH_SIZE): #for i in tqdm(range(0, len(X), BATCH_SIZE)):
			X_batch_a = X_a[i:i+BATCH_SIZE].view(-1, 1, max_len_features, 23) #.view(-1, 1, max_len_features, 23) for CNN
...
#creates a list of arrays (since each array has a different shape due to audiolength)
def list_arrays(x):
	listed = []
	for i in x:
		for j in i:
...
def standardize(features, mean, std, video=False):
	...
	"""new_mean, new_std = mean_std(features)
...
for i in range(len(l)):
	...
	#print(f' X_train is: {len(X_train)} X_test is: {len(X_test)}')
	#print(f'y_train is: {y_train} y_test is: {y_test}')
	...
	#print(f' X_test_a shape is: {X_test_a.shape} y_test length is: {len(y_test)}')
	#print(f' X_test_v shape is: {X_test_v.shape} y_test length is: {len(y_test)}')
